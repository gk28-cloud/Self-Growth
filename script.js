...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('exportWidgetBtn')?.addEventListener('click', exportWidgetData);

});

/* ─── WIDGET EXPORT ─── */
function exportWidgetData() {
  // Build the summary JSON that the Scriptable widget will read
  const today = todayStr();

  const habitsData = state.habits.map(h => {
    const streak  = calcStreak(h.id);
    const longest = calcLongestStreak(h.id);
    const score   = disciplineScore(h.id);
  ...
  const summary = {
    ...
    doneToday:        state.habits.filter(h => isComplete(h.id, today)).length,
    todayPct:         todayCompletionPct(),
    disciplineScore:  globalDisciplineScore(),
    currentStreak:    Math.max(0, ...state.habits.map(h => calcStreak(h.id))),
    longestStreak:    globalLongestStreak(),
    missedThisWeek:   globalMissedThisWeek(),
    habits:           habitsData,
  };
  ...
  a.href         = url;
  a.download     = 'self-growth-widget.json';
  a.click();
  URL.revokeObjectURL(url);

  // Show confirmation toast
  showToast('Widget data exported! Save to iCloud Drive → Scriptable folder.');
}
